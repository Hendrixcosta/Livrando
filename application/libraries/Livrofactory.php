...
class Livrofactory {
    ...
    public function sauda() {
     ...
     public function getLivro($id = 0) {
        ...
        if ($id > 0) {
            //Getting an individual livro
            $query =  $this->_ci->db->query("SELECT * FROM bookdescriptions a JOIN bookauthorsbooks b ON a.ISBN = b.ISBN JOIN bookauthors c ON b.AuthorID = c.AuthorID JOIN bookcategoriesbooks d ON d.ISBN=a.ISBN JOIN bookcategories e ON d.CategoryID=e.CategoryID WHERE a.ISBN=" . $id. " ;");
            ...
            //$query =  $this->_ci->db->query("SELECT (a.ISBN), a.title, c.nameF, c.nameL, e.CategoryName  FROM bookdescriptions a JOIN bookauthorsbooks b ON a.ISBN = b.ISBN JOIN bookauthors c ON b.AuthorID = c.AuthorID JOIN bookcategoriesbooks d ON d.ISBN=a.ISBN JOIN bookcategories e ON d.CategoryID=e.CategoryID GROUP BY (a.ISBN);");
